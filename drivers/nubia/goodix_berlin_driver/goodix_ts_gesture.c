...
//+linx
#define IS_CMD_SUCCESS(ret) (0 == (ret))
#define SET_CMD(ts_cmd, cmd_data) do{memcpy(&(ts_cmd).buf[2], (cmd_data), sizeof(cmd_data));}while(0)
#define RUN_CMD(module, ts_cmd, err_msg, ret) do{\
if(0 == ((ret) = (module)->core_data->hw_ops->send_cmd((module)->core_data, &(ts_cmd)))){ ts_err(err_msg); } \
}while(0)
//-linx

//+0016005364 20221216 for charger_mode
static int charger_mode = 0;

static ssize_t charger_mode_show(struct goodix_ext_module *module,
		char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d\n",  charger_mode );
}

static	ssize_t charger_mode_store(struct goodix_ext_module *module,
			const char *buf, size_t count)
{
    int ret = 0;
	int mode = 0;
	struct goodix_ts_cmd cmd;
    unsigned char enableChargermodeCmdData[]  = {0x05, 0xAF, 0x01};
    unsigned char disableChargermodeCmdData[] = {0x05, 0xAF, 0x00};
    ...
    if(mode == charger_mode){
        ts_info("same mode, skip");
        return -1;
    }

    switch(mode){
    case 0: //disable charger mode
        SET_CMD(cmd, disableChargermodeCmdData);
        RUN_CMD(module, cmd, "disable charger mode", ret);
        break;
    default: //enable charger mode
        SET_CMD(cmd, enableChargermodeCmdData);
        RUN_CMD(module, cmd, "enable charger mode", ret);
        break;
    }
    if(IS_CMD_SUCCESS(ret)){
        charger_mode = mode;
        ts_info("%s charger mode success", mode ? "enable" : "disable");
        return count;
    }
    return -1;
}
			...
			const char *buf, size_t count)
{
	...
	int mode = 0;
	struct goodix_ts_cmd highrate;
    unsigned char exit960HzCmdData[]  = {0x06, 0xC1, 0x00, 0x00};
    unsigned char exit480HzCmdData[]  = {0x06, 0xC0, 0x00, 0x00};
    unsigned char enter480HzCmdData[] = {0x06, 0xC0, 0x01, 0x00};
    unsigned char enter960HzCmdData[] = {0x06, 0xC1, 0x01, 0x00};

	sscanf(buf, "%x", &mode);
	NX709_highrate_data = mode;
	ts_info("NX709_highrate:%d", mode);

    switch(mode){
    case 0: //240Hz
        SET_CMD(highrate, exit960HzCmdData);
        RUN_CMD(module, highrate, "exit 960Hz cmd", ret);
        SET_CMD(highrate, exit480HzCmdData);
        RUN_CMD(module, highrate, "exit 480Hz cmd", ret);
        ts_info("enter 240Hz");
        break;
    case 1: //480Hz
        SET_CMD(highrate, exit960HzCmdData);
        RUN_CMD(module, highrate, "exit 960Hz cmd", ret);
        SET_CMD(highrate, enter480HzCmdData);
        RUN_CMD(module, highrate, "enter 480Hz cmd", ret);
        ts_info("enter 480Hz");
        break;
    default://960Hz
        SET_CMD(highrate, enter480HzCmdData);
        RUN_CMD(module, highrate, "enter 480Hz cmd", ret);
        SET_CMD(highrate, enter960HzCmdData);
        RUN_CMD(module, highrate, "enter 960Hz cmd", ret);
        ts_info("enter 960Hz");
        break;
    }
	return count;
...
const struct goodix_ext_attribute gesture_attrs[] = {
    ...
    //+0016005364 20221216 for charger_mode
	__EXTMOD_ATTR(charger_mode, 0666, charger_mode_show, charger_mode_store),
    //-0016005364
