...
#define GOODIX_CORE_DRIVER_NAME		"nubia_goodix_ts"
...
struct goodix_ts_board_data {
	...
	char ovdd_name[GOODIX_MAX_STR_LABLE_LEN];
	int reset_gpio;
	int irq_gpio;
	int avdd_gpio;
	int iovdd_gpio;
...
struct goodix_ts_event {
	...
	bool proximity;  /* indicate proximity state */
	u8 gesture_type;
...
struct goodix_ts_hw_ops {
	...
	int (*proximity)(struct goodix_ts_core *cd, bool enable);
...
extern struct goodix_ts_core *core_data; //wzm
