...
static DEFINE_MUTEX (spong_mutex);
...
{
	struct goodix_ts_core *cd = client_data;
	...
	ts_info("fp_switch:%d", core_data->fp_switch);
//		ts_info("en_fpmode:%d", core_data->en_fpmode);
	if(core_data->fp_switch) {
		ts_err("wzm notification out");
		return;
	}
	...
	case DRM_PANEL_EVENT_UNBLANK:
		if (notification->notif_data.early_trigger)
			ts_info("resume notification pre commit");
		else
	...
	case DRM_PANEL_EVENT_BLANK:
		if (notification->notif_data.early_trigger) {
			...
			ts_info("suspend notification post commit");
		}
		break;
	...
	case DRM_PANEL_EVENT_FPS_CHANGE:
		ts_info("shashank:Received fps change old fps:%d new fps:%d",
...
/* enter proximity mode */
static ssize_t goodix_ts_proximity_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf,
				     size_t count)
{
	...
	if (buf[0] == '0')
		ret = hw_ops->proximity(core_data, false);
	else
		ret = hw_ops->proximity(core_data, true);
						...
						const char *buf, size_t count)
{
    ...
    ts_info("goodix_ts_send_cfg_store IN");
	ret = goodix_send_ic_config(core_data, en);

	hw_ops->irq_enable(core_data, true);
    ts_info("goodix_ts_send_cfg_store out");
	return count;
}
...
static  ssize_t synaptics_nubia_fpswitch_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
static  ssize_t synaptics_nubia_bsp_mode_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
static  ssize_t synaptics_nubia_mode_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);


static DEVICE_ATTR(fpswitch, S_IRUGO | S_IWUSR | S_IWGRP,
		   NULL, synaptics_nubia_fpswitch_store);
static DEVICE_ATTR(bsp_mode, S_IRUGO | S_IWUSR | S_IWGRP,
		   NULL, synaptics_nubia_bsp_mode_store);
static DEVICE_ATTR(mode, S_IRUGO | S_IWUSR | S_IWGRP,
		   NULL, synaptics_nubia_mode_store);
...

{
	...
	//struct goodix_ts_core *cd = client_data;
	unsigned int  en ;
	mutex_lock(&spong_mutex);

	sscanf(buf, "%d", &en);
    core_data->en_fpmode = en ;
	ts_info("fp_mode:%d", en);
		if(core_data->bUpdateOver){
		msleep(10000);
		ts_info("success bUpdateOverk");
			}
    ...
    schedule_work(&core_data->resume_work);
	 break;
	case FP_LCD_OFF:
    schedule_work(&core_data->suspend_work);
     break;
    case NORMAL_GESTURE:
    schedule_work(&core_data->suspend_work);
     break;
...
mutex_unlock(&spong_mutex);
	return count;
}

//static ssize_t synaptics_nubia_bsp_mode_show(struct device *dev,
//				       struct device_attribute *attr,
//				       char *buf)
//{
//	return 0;
...
static inline ssize_t synaptics_nubia_bsp_mode_store(struct device *dev,
                struct device_attribute *attr, const char *buf, size_t count)
...

{
	...
	unsigned int  en ;
	sscanf(buf, "%d", &en);
    core_data->en_bspmode = en;
	ts_info("bsp_mode:%d", en);
		if(core_data->fp_switch) {
		ts_err("bsp notification out");
		return count;
	}
	if(core_data->bUpdateOver){
		msleep(10000);
...
static inline ssize_t synaptics_nubia_mode_store(struct device *dev,
                struct device_attribute *attr, const char *buf, size_t count)
...
if(input == core_data->fp_switch) {
	ts_err(" Failed: rewrite %d to fp_switch", input);
	return count;
}
core_data->fp_switch = input;

ts_err("->%s  core_data->fp_switch = %d",  core_data->fp_switch ?"error set":"success set", input);
...
extern bool fp_switch;

static DEVICE_ATTR(proximity, 0220,	NULL, goodix_ts_proximity_store);
...
static struct attribute *sysfs_attrs[] = {
	...
	&dev_attr_bsp_mode.attr,
	&dev_attr_mode.attr,
	&dev_attr_fpswitch.attr,
...
__ATTR(fp_mode,   0664,   synaptics_nubia_fp_mode_show,  synaptics_nubia_fp_mode_store),
};
...
static int synaptics_nubia_sysfs_node(void)
{
	...
	ts_info("sys/kernel/tp_node ->create start\n");

	tp_kobj = kobject_create_and_add("tp_node", kernel_kobj);
	if (!tp_kobj) {
		ts_info("failed to create and add kobject\n");
		return -ENOMEM;
	}
	for (attr_count = 0; attr_count < ARRAY_SIZE(tp_node_attrs); attr_count++) {
		...
		if (retval < 0) {
			ts_info("failed to create sysfs attributes\n");
			goto err_sys_creat;
		}
	}

	ts_info("sys/kernel/tp_node ->create complete\n");
...
//#endif
	...
	if (r < 0) {
		ts_err("invalid iovdd-gpio in dt: %d", r);
		return -EINVAL;
	}
	...
	memset(board_data->ovdd_name, 0, sizeof(board_data->ovdd_name));
	r = of_property_read_string(node, "goodix,ovdd-name", &name_tmp);
	if (!r) {
		ts_info("ovdd name from dt: %s", name_tmp);
		if (strlen(name_tmp) < sizeof(board_data->ovdd_name))
			strlcpy(board_data->ovdd_name,
				name_tmp, sizeof(board_data->ovdd_name));
		else
			ts_info("invalied iovdd name length: %ld > %ld",
				strlen(name_tmp),
				sizeof(board_data->ovdd_name));
	}
	...
	if (count > 0) {
		for (i = 0; i < count; i++) {
			panel_np = of_parse_phandle(node, "panel", i);
...
//TODO change key code as you wanted
#define GOODIX_PROXIMITY_KEY KEY_F14
static int goodix_ts_proximity_report(struct input_dev *dev, bool state)
{
	static bool pre_proximity_state;

	if (state != pre_proximity_state) {
		if (state)
			input_report_key(dev, GOODIX_PROXIMITY_KEY, 1);
		else
			input_report_key(dev, GOODIX_PROXIMITY_KEY, 0);
		input_sync(dev);
	}
	pre_proximity_state = state;
...
static irqreturn_t goodix_ts_threadirq_func(int irq, void *data)
{
	...
	if (likely(!ret)) {
		goodix_ts_proximity_report(core_data->input_dev, ts_event->proximity);
...
static int goodix_ts_power_init(struct goodix_ts_core *core_data)
{
	...
	if (strlen(ts_bdata->ovdd_name)) {
		core_data->ovdd = devm_regulator_get(dev,
				 ts_bdata->ovdd_name);
		if (IS_ERR_OR_NULL(core_data->ovdd)) {
			ret = PTR_ERR(core_data->ovdd);
			ts_err("Failed to get regulator ovdd:%d", ret);
			core_data->ovdd = NULL;
	}
	} else
...
//	}
	...
	if (r < 0) {
		...
		ts_info("GPIO setup iovdd_gpio:%d", ts_bdata->iovdd_gpio);
...
static int goodix_ts_input_dev_config(struct goodix_ts_core *core_data)
{
	...
	input_set_capability(input_dev, EV_KEY, GOODIX_PROXIMITY_KEY);
...
out:
	...
	core_data->goodix_suspend = true;
	ts_info("goodix_suspend:%d", core_data->goodix_suspend);
	if (core_data->enable_wakeup_gesture) {
		enable_irq_wake(core_data->irq);
	ts_info("core_data->enable_wakeup_gesture:%x", core_data->enable_wakeup_gesture);
    }
	ts_info("Suspend end");
	return 0;
}
...
out:
	...
	core_data->goodix_suspend = false;
	ts_info("goodix_suspend:%d", core_data->goodix_suspend);
	ts_info("Resume end");
	return 0;
}
...
#ifdef CONFIG_DRM
	if (active_panel) {
		cd->notifier_cookie = panel_event_notifier_register(PANEL_EVENT_NOTIFICATION_PRIMARY,
				PANEL_EVENT_NOTIFIER_CLIENT_PRIMARY_TOUCH, active_panel,
				&goodix_ts_panel_notifier_callback, cd);
		if (!cd->notifier_cookie)
...
static int goodix_later_init_thread(void *data)
{
	...
	core_data->bUpdateOver = false;
	ts_info("ts_core_data->bUpdateOver:%x", core_data->bUpdateOver);
...
static void goodix_resume_work_func(struct work_struct *work)
{
	struct goodix_ts_core *core_data =
			container_of(work, struct goodix_ts_core,
			resume_work);
        goodix_ts_resume(core_data);
}


static void goodix_suspend_work_func(struct work_struct *work)
{
	struct goodix_ts_core *core_data =
			container_of(work, struct goodix_ts_core,
			suspend_work);
...
static int goodix_ts_probe(struct platform_device *pdev)
{
	...
	goodix_ts_sysfs_init(core_data);
	   ts_info("goodix_ts_sysfs_init probe success");

	core_data->init_stage = CORE_INIT_STAGE1;
	core_data->goodix_suspend = false;
	core_data->en_fpmode = false;//wzm
	core_data->en_bspmode = false;//wzm
	core_data->fp_switch = false;
	goodix_modules.core_data = core_data;
	core_module_prob_sate = CORE_MODULE_PROB_SUCCESS;

		/* gesture init */
	gesture_module_init(core_data);
		  ts_info("gesture_module_init probe success");

	synaptics_nubia_sysfs_node();
	INIT_WORK(&core_data->resume_work, goodix_resume_work_func);
	INIT_WORK(&core_data->suspend_work, goodix_suspend_work_func);
