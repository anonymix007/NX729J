...
static int brl_power_on(struct goodix_ts_core *cd, bool on)
{
	...
	if (on) {
		...
		//wzm
		ret = regulator_enable(cd->ovdd);
		if (ret < 0) {
				ts_err("Failed to enable ovdd:%d", ret);
			}
	usleep_range(15000, 15100);
			ret =gpio_direction_output(cd->board_data.iovdd_gpio, 1);
			usleep_range(3000, 3100);
	        ts_err("gpio_direction_output to able iovdd:%d", ret);
			//gpio_direction_output(cd->board_data.avdd_gpio, 1);
		    //usleep_range(15000, 15100);
		...
		*/
		...
		if (ret < 0) {
			...
			ret =gpio_direction_output(cd->board_data.iovdd_gpio, 0);
            ts_err(" wzmgpio_direction_output to disable iovdd:%d", ret);
   ...
   ret = regulator_disable(cd->avdd);
	//ret = gpio_direction_output(cd->board_data.avdd_gpio, 0);
	ts_err("gpio_direction_output to disable iovdd:%d", ret);
	if (!ret)
		ts_info("regulator disable SUCCESS");
	else
...
int brl_suspend(struct goodix_ts_core *cd)
{
	...
	sleep_cmd.len = 4;
	ts_err("enable_wakeup_gesture %d",cd->enable_wakeup_gesture);
	if(cd->enable_wakeup_gesture){
...
int brl_gesture(struct goodix_ts_core *cd, int gesture_type)
{
    ...
    struct goodix_ts_cmd NX709_cmd;
    ...
    if (cd->hw_ops->send_cmd(cd, &NX709_cmd))
		ts_err("failed send gesture cmd");
...
#define GOODIX_PROXIMITY_CMD	0x93
int brl_proximity(struct goodix_ts_core *cd, bool enable)
{
	struct goodix_ts_cmd cmd;

	if (cd->bus->ic_type == IC_TYPE_BERLIN_A) {
		ts_err("proximity unsupported");
		return -EINVAL;
	}
	cmd.cmd = GOODIX_PROXIMITY_CMD;
	cmd.len = 5;
	if (enable)
		cmd.data[0] = 0x03;
	else
	...
	if (cd->hw_ops->send_cmd(cd, &cmd)) {
		ts_err("failed send proximity cmd");
		return -EIO;
	}
	ts_info("success send proximity %s cmd", enable ? "enter" : "exit");
	return 0;
}
...
#define GOODIX_PROXIMITY_EVENT      0x20
				...
				u8 *pre_buf, u32 pre_buf_len)
{
	...
	/* process proximity info */
	if (buffer[2] & GOODIX_PROXIMITY_EVENT)
		ts_event->proximity = true;
	else
		ts_event->proximity = false;
