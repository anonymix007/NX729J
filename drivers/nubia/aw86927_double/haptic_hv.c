...
char *aw_ram_name = "haptic_ram_709.bin";
#else
...
static void audio_off(struct aw_haptic *aw_haptic)
{
	...
	left->func->set_gain(aw_haptic, 0x80);
	right->func->set_gain(aw_haptic, 0x80);
#else
...
aw_info("f0_cali_data = 0x%02X", aw_haptic->f0_cali_data);
aw_info(" f0_cali_data = %d", aw_haptic->f0_cali_data);
	return len;
}
